...
<div class="mt-4 w-full">
	<h2
		class="mb-4 text-2xl font-bold transition-colors duration-300"
	...
	{#if unlockedAchievements.length > 0}
		<div class="space-y-4">
			{#each unlockedAchievements as achievement (achievement.id)}
				...
				>
					<h3 class="mb-2 text-lg font-bold">{achievement.name}</h3>
	...
	{:else}
		<p
			class="text-center italic transition-colors duration-300"
