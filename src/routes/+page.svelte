...
<div class="min-h-screen py-12 px-4 sm:px-6 lg:px-8 transition-colors duration-300"
  ...
  <main class="max-w-4xl mx-auto text-center">
      <h1 class="mb-8 text-6xl font-extrabold transition-colors duration-300" 
          class:text-indigo-800={!$darkMode}
          class:text-indigo-300={$darkMode}
          in:fly={{ y: -50, duration: 1000, easing: quintOut }}>
          ひらがな＆カタカナ
          <span class="block text-3xl font-normal mt-2 transition-colors duration-300"
      ...
      <div class="grid grid-cols-1 md:grid-cols-2 gap-6 mb-12">
          {#each navigationItems as { title, route, color }, index}
              <button
                  on:click={() => goto(route)}
                  class="p-6 rounded-2xl text-white text-xl font-semibold shadow-lg transition-all duration-300 transform hover:scale-105 focus:outline-none focus:ring-4 focus:ring-offset-2"
      ...
      <div class="rounded-3xl shadow-2xl p-8 transition-colors duration-300 w-full"
